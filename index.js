...
function filterFundedOnly() {
    ...
    const filteredFunded = GAMES_JSON.filter((game)=>{
        return game.pledged>=game.goal;
    })
    

    // use the function we previously created to add unfunded games to the DOM
    addGamesToPage(filteredFunded)
    console.log(filteredFunded.length)
...
const descriptionString = `A total of $${totalRaised.toLocaleString()} has been raised for ${GAMES_JSON.length} games. Currently, ${numberOfUnfundedGames} ${numberOfUnfundedGames === 1 ? "game remains" : "games remain"} unfunded. We need your help to fund these amazing games!`;
